...
class CardReader:
    ...
    CommandReg = (
        0x01
    )  # set to PCD_RESETPHASE(0F)- SoftReset - resets the MFRC522
    CommIEnReg = 0x02
    DivlEnReg = 0x03
    CommIrqReg = 0x04
    DivIrqReg = 0x05
    ...
    def AntennaOn(self):
        temp = self.Read_MFRC522(
            self.TxControlReg
        )  # (bit 1) Tx2RFEn - Transmission Tx2  to RF enabled
    ...
    def MFRC522_Request(self, reqMode):  # scan card reqMode is 0x26
        ...
        self.Write_MFRC522(
            self.BitFramingReg, 0x07
        )  # adjusments for bit-oriented frames
    ...
    def MFRC522_Init(self):
        ...
        self.Write_MFRC522(
            self.CommandReg, self.PCD_RESETPHASE
        )  # Command to initiate a Software Reset

        self.Write_MFRC522(
            self.TModeReg, 0x8D
        )  # 0100 1101 - Define the Timer Settings
        ...
        self.Write_MFRC522(
            self.TxASKReg, 0x40
        )  # controls the Setting of the Transmission Regulation
    ...
    def scan_card(self):
        ...
        # Scan for cards
        (status, TagType) = self.MFRC522_Request(self.PICC_REQIDL)  # 0x26
