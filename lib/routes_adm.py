...
def start_server():
    ...
    def form():
        ...
        if not session.get('logged_in'):
            ...
            return render_template('form_adm.html', IP=str(get_ip()),
                                    port=3000,
                                    tz_dic=tz_sorted)
    ...
    def result():
        if request.method == 'POST':
            ...
            with open(WORK_DIR + 'dicts/data_adm.json', 'w+') as outfile:
                json.dump(dic, outfile)
            return render_template("result_adm.html", result=results)
